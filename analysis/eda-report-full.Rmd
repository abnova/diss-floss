...
```{r getBarChartsInfo}
refInfo <- genObjRefs("fig", "barchart_")
```

In order to gain some understanding about _data patterns_, the data set’s discrete measures were summarized via _frequency tables_ (containing frequency scores across categorical variables), which can be found in Appendix TODO, as well as visually, using bar charts, as shown in `r refInfo$str` below.

```{r barchartDynamicChunk, message=TRUE}
barchartChunks <- genDynChunks(refInfo)
```

`r knit(text = barchartChunks)`

```{r getHistogramsInfo}
refInfo <- genObjRefs("fig", "histogram_")
```

Visual overview for continuous (binned) data is presented by histograms in the following `r refInfo$str`.

```{r historgramsDynamicChunk, message=TRUE}
histChunks <- genDynChunks(refInfo)
```
