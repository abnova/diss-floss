...
latexObjLabel <- paste0("{{caption}}", "\\\\label{", bcRefStr$objType, ":{{varName}}", "}")
...
figures <- lapply(bcRefStr$objs, function (x) knit_expand(text = chunkText, name = x, name2 = gsub('\\.', '_', x), varName = strsplit(x, bcRefStr$objTypePrefix)[[1]][2], caption = attr(get(x), 'title')))
```

`r knit(text = unlist(figures))`
