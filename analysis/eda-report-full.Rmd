...
```{r DynamicChunk, message=TRUE}
chunkName <- "{{name2}}"
chunkHeader <- paste0("```{r ", chunkName, ",")
chunkOptions <- "include=TRUE, results='asis', fig.height=4, fig.width=4, fig.cap='EDA'}"
chunkHeaderFull <- paste(chunkHeader, chunkOptions)
chunkBody <- "print(get('{{name}}'))"

latexObjLabel <- paste0("\n", "cat('\\\\label{", bcRefStr$objType, ":{{name}}", "}')")

chunkText <- c(chunkHeaderFull, chunkBody, latexObjLabel, "```")
