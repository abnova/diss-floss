...
```{r DynamicChunk}
chunkText <- "
\\`\\`\\`{r {{name}}, include=TRUE, results='asis', fig.height=4, fig.width=4, fig.cap='EDA: Barcharts'}
#for (g in bcRefStr$objs) print(get(g))
print(get({{name}}))
\\`\\`\\`
"
for (x in bcRefStr$obj) knit_expand(text = chunkText, name = x)
```

In order to gain some understanding about _data patterns_, the data set’s discrete measures were summarized via _frequency tables_ (containing frequency scores across categorical variables), which can be found in Appendix TODO, as well as visually, using bar charts and - for continuous (binned) data - histograms, as shown in the tables below.
