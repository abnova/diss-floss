...
```{r DynamicChunk}
chunkName <- "sub('\\\\.', '_', {{name}})"
chunkHeader <- paste("```{r", chunkName, ",")
...
chunkBody <- "print(get(deparse(substitute({{name}}))))"

chunkText <- c(chunkHeaderFull, chunkBody, "```")

figReportParts <- lapply(bcRefStr$obj, function (x) knit_expand(text = chunkText, name = x))
```
